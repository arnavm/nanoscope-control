...
class TwoSpotFocusFunctionality(hardwareModule.HardwareFunctionality, lockModule.QPDCameraFunctionalityMixin):
    qpdUpdate = QtCore.pyqtSignal(dict)
    # threadUpdate = QtCore.pyqtSignal(dict)  # not sure we need this
...
class PointGreyLockCameraTwoSpot(hardwareModule.HardwareModule):
    """
    HAL module that interfaces with a Point Grey camera (for a focus lock) as a QPD style (uc480 style)
    focus locks. 
    """
    def __init__(self, module_params = None, qt_settings = None, **kwds):
        super().__init__(**kwds)
        ...
        self.camera = pointGreyLockCamera.CameraQPD(camera_id = configuration.get("camera_id"),
                                                       parameters = configuration.get("camera_parameters"))
        self.camera_functionality = TwoSpotFocusFunctionality(camera = self.camera,
                                                                parameters = configuration.get("parameters"),
                                                                units_to_microns = configuration.get("units_to_microns"))
